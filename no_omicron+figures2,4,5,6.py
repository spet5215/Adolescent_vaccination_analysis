...
if __name__ == '__main__':
    ...
    daily_infectious = sc.objdict({
        'No Omicron Scenario: Daily Number of Infectious People': ['new_infectious']})
    
    
    daily_infectious_by_variant = sc.objdict({
        'No Omicron Scenario: Daily Number of Infectious People by Variant': ['new_infectious_by_variant']})
    
    #saving the sims
    
    msim.to_excel('Scenario_2.xlsx')
    
    #plotting the sims
    
    # plot the number of infectious people by variant between September 2020 and January 2022, for the scenario in which omicron is excluded.
    
    msim.plot(to_plot = daily_infectious_by_variant, style='bmh', do_save = False, 
              fig_path = 'Figure_5.png', do_show = False, 
    ...
    pl.savefig('Figure_5.png')
    
    # plot the number of infectious between September 2020 and January 2022, for the scenario in which omicron is excluded.
    
    msim.plot(to_plot = daily_infectious, style='bmh', do_save = True, 
              fig_path = 'Figure_4.png', do_show = False, 
              tight = True, fig_args = {'num':'new_infectious'}, 
              start = '2021-05-01', end = '2022-01-01', commaticks = True, 
              figsize = (10,4), dpi = 150, labels = 'Daily Number of Infectious People',
              legend_args = {'show_legend': False})
    pl.gca().set_ylim([0, 500000])
    pl.savefig('Figure_4.png')

    # plot the number of infectious people between September 2020 and January 2022 of both scenarios together (with omicron and without omicron)    

    data = pd.read_excel('Scenario_1.xlsx')
    data2 = pd.read_excel('Scenario_2.xlsx')

    pl.gca().set_ylim([0, 500000])
    ax=pl.gca()
    ax.plot(data.date[467:800], data.new_infectious[467:800], color = 'orange', label = 'With omicron', alpha = 0.8)
    ax.plot(data.date[467:800], data.new_infectious_low[467:800], color = 'orange', linewidth = 0.0025, alpha = 0.5)
    ax.plot(data.date[467:800], data.new_infectious_high[467:800], color = 'orange', linewidth = 0.0025, alpha = 0.5)
    ax.fill_between(data.date[467:800], data.new_infectious_low[467:800], 
                          data.new_infectious_high[467:800], alpha = 0.15, color = 'orange')
    #ax.legend(('Without Omicron','Omicron'), framealpha = 0, loc = 'upper left')
    handles, labels = ax.get_legend_handles_labels()
    labels[0] = 'Without omicron'
    ax.legend(handles=handles, labels=labels, loc = 'upper left')
    pl.savefig('Figure_2.png')
    
    # plot the increase in the daily number of infectious people with the introduction of omicron compared to when we didnt introduce it.
    
    pl.figure(figsize = (10,4), dpi = 150)
    pl.bar(data.date, data.new_infectious-data2.new_infectious)
    pl.style.use('bmh')
    pl.gca().set_title('Increase in the Daily Number of Infectious People with the Introduction of the Omicron Variant', fontsize = 12)
    sc.dateformatter( dateformat = '%b', start = '2021-10-15', end = '2022-01-01')
    pl.gca().xaxis.set_major_locator(mdates.MonthLocator(bymonth=(7,8,9,10,11,12,1)))
    pl.gca().set_ylim([0, 300000])
    pl.gca().set_yticklabels(['{:,}'.format(int(x)) for x in pl.gca().get_yticks().tolist()])
    pl.tight_layout()
